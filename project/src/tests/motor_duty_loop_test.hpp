...
constexpr float DUTY_CYCLE_MAX = 1.0f;
...
class motor_duty_loop_test : public fp_task {
public:
  void init() override;
...
private:
  ...
  fp_motor motor_right = fp_motor(ZF_PWM_MOTOR_1, ZF_GPIO_MOTOR_2);
...
void motor_duty_loop_test::init() {
  motor_left.init();
  motor_right.init();
  timer.init_ms(TIMER_INTERVAL, [] {});
}

void motor_duty_loop_test::deinit() {
  timer.stop();
  motor_left.deinit();
  motor_right.deinit();
}

void motor_duty_loop_test::work() {
  while (true) {
