...
static inline void control_manager_init(control_manager& ctrl)
{
    ...
    // 差速解算初始化
    diff_drive_init(ctrl.drive, params.diff_drive.target_limit);
...
// 获取目标角速度
static inline float control_manager_get_yaw_rate_target(control_manager& ctrl)
{
...
// 获取实际角速度
static inline float control_manager_get_yaw_rate_current(control_manager& ctrl)
{
...
static inline void control_manager_loop(control_manager& ctrl, float dt_s)
{
    ...
    base_speed_plan = ctrl.base_speed_target
                    - ctrl.turn_slow_gain * turn_for_speed_plan;
